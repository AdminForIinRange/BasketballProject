...
export default function VideoEditor() {
  const [tracks, setTracks] = useState<Track[]>([
    {
      ...
      name: "Track A",
      clips: [
        { id: "1", person: "Audio 1", duration: 120, startTime: 0, content: "Welcome to our podcast...", color: "blue" },
        { id: "2", person: "Audio 2", duration: 80, startTime: 140, content: "Thanks for having me...", color: "green" },
        { id: "4", person: "Person 4", duration: 60, startTime: 240, content: "That's interesting...", color: "purple" },
      ],
    },
    {
      id: "B",
      name: "Track B",
      clips: [
        { id: "3", person: "Audio 3", duration: 100, startTime: 20, content: "I think we should...", color: "orange" },
        { id: "5", person: "Person 5", duration: 150, startTime: 140, content: "Let me explain this...", color: "teal" },
        { id: "6", person: "Person 6", duration: 90, startTime: 310, content: "Great point about...", color: "red" },
      ],
    },
  ])
  ...
  const resolveOverlapsGlobally = useCallback((allTracks: Track[]): Track[] => {
    ...
    const flat = allTracks.flatMap((t) => t.clips.map((c) => ({ ...c, __trackId: t.id } as AudioClip & { __trackId: string })))
    ...
    for (const clip of flat) {
      ...
      const finalStart = adjusted.length === 0 ? clip.startTime : Math.max(clip.startTime, newStart)
    ...
    return allTracks.map((t) => ({
      ...t,
      clips: adjusted.filter((c) => c.__trackId === t.id).map(({ __trackId, ...rest }) => rest),
    }))
  }, [])

  const processedTracks = useMemo(() => resolveOverlapsGlobally(tracks), [tracks, resolveOverlapsGlobally])

  const totalDuration = useMemo(() => {
    const ends = processedTracks.flatMap((track) => track.clips.map((clip) => clip.startTime + clip.duration))
  ...
  const handleClipClick = useCallback((clip: AudioClip) => setSelectedClip(clip), [])

  const updateClipContent = useCallback((clipId: string, newContent: string) => {
    setTracks((prev) =>
      prev.map((track) => ({
        ...track,
        clips: track.clips.map((clip) => (clip.id === clipId ? { ...clip, content: newContent } : clip)),
      })),
    )
  }, [])
  ...
  const playheadLeft = useMemo(() => getClipLeft(currentTime), [currentTime, getClipLeft])
  ...
  const renderTrack = useCallback(
    (track: Track) => (
      <HStack key={track.id} align="stretch" spacing={4}>
        ...
        <Box position="relative" bg="gray.50" border="1px solid" borderColor="gray.200" borderRadius="md" px="0" py={`${timelinePaddingY}px`} width={`${timelineWidth}px`} onClick={onScrub}>
          {/* grid & ruler */}
          <Box position="relative" height={`${laneHeight}px`}>
            ...
            {track.clips.map((clip) => (
              <Box
                ...
                bg={selectedClip?.id === clip.id ? `${clip.color}.500` : `${clip.color}.400`}
                border="2px solid"
                borderColor={selectedClip?.id === clip.id ? `${clip.color}.700` : `${clip.color}.500`}
                borderRadius="md"
                boxShadow="sm"
                cursor="pointer"
  ...
  return (
    <VStack spacing={8} p={8} align="stretch" maxW="1200px" mx="auto" bg="white" minH="100vh">
      ...
      <HStack spacing={6} justify="space-between" p={4} bg="gray.100" borderRadius="lg">
        <HStack spacing={4}>
          <Button colorScheme="blue" onClick={handlePlayPause} size="lg" leftIcon={<Text>{isPlaying ? "⏸️" : "▶️"}</Text>}>
            {isPlaying ? "Pause" : "Play"}
          </Button>
          <Box bg="white" px={4} py={2} borderRadius="md" border="1px solid" borderColor="gray.300" minW="170px" textAlign="center">
            <Text fontSize="lg" fontWeight="semibold">
              {formatTime(currentTime)} / {formatTime(totalDuration)}
            </Text>
          </Box>
        </HStack>
        <Text fontSize="sm" color="gray.600">Click the timeline to scrub</Text>
      </HStack>
      ...
      {selectedClip && (
        <Box p={6} border="1px solid" borderColor="gray.200" borderRadius="xl" bg="gray.50">
          ...
          <VStack spacing={4} align="stretch">
            <HStack spacing={6}>
              <Box bg="white" p={3} borderRadius="md" border="1px solid" borderColor="gray.200" minW="140px">
                <Text fontSize="xs" color="gray.600">Duration</Text>
                <Text fontSize="lg" fontWeight="bold">{selectedClip.duration}s</Text>
              </Box>
              <Box bg="white" p={3} borderRadius="md" border="1px solid" borderColor="gray.200" minW="140px">
                <Text fontSize="xs" color="gray.600">Start Time</Text>
                <Text fontSize="lg" fontWeight="bold">{selectedClip.startTime}s</Text>
              </Box>
            </HStack>

            <VStack align="stretch">
              <Text fontWeight="semibold" color="gray.700">Content</Text>
              <Textarea
                value={selectedClip.content}
                onChange={(e) => updateClipContent(selectedClip.id, e.target.value)}
      ...
      <Box p={6} border="1px solid" borderColor="gray.200" borderRadius="xl" bg="gray.50">
        ...
        <VStack spacing={3} align="stretch">
          {processedTracks.map((track) => (
            <HStack key={track.id} justify="space-between" p={3} bg="white" borderRadius="md" border="1px solid" borderColor="gray.100">
              <Text fontWeight="semibold">{track.name}</Text>
              <Text color="gray.600">{track.clips.length} clips</Text>
            </HStack>
          ))}
          <HStack justify="space-between" p={3} bg="white" borderRadius="md" border="1px solid" borderColor="gray.200">
            <Text fontWeight="bold" fontSize="lg">Total Duration</Text>
            <Text fontWeight="bold" fontSize="lg" color="blue.600">{formatTime(totalDuration)}</Text>
          </HStack>
        </VStack>
      </Box>
    </VStack>
